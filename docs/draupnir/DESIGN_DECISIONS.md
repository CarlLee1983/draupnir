...
| 決策 | 選擇 | 理由 |
|------|------|------|
| **多租戶隔離策略** | 列級隔離（Row-Level Security）+ `organization_id` 篩選 | 實作單純，適合現階段規模 |
...
| 決策 | 選擇 | 理由 |
|------|------|------|
| **時間控制** | `IClock` 依賴注入 | 允許測試中使用 `TestClock` 模擬時間流逝，確保 Token 過期、排程等邏輯可測試且不依賴系統時間 |
| **日誌格式** | 結構化 JSON (Pino 格式) | 方便 ELK / CloudWatch 收集與檢索，保留 Context ID 追蹤請求 |
| **健康檢查** | 模組化 Health Checks | 各模組自定義檢查點（DB, Redis, API），供 Kubernetes 或監控系統判斷可用性 |
