...
├── Http/
│   ├── Inertia/
│   │   ├── createInertiaRequestHandler.ts    ← 原 inertiaFactory.ts
│   │   ├── InertiaRequestHandler.ts   ← 原 InertiaService.ts
│   │   ├── withInertiaPage.ts
│   │   └── SharedPropsBuilder.ts       ← 原 SharedDataMiddleware.ts
│   ├── Routing/
│   │   ├── bindPageAction.ts
...
│   │   └── ViteTagHelper.ts
│   └── Security/
│       └── CsrfMiddleware.ts        ← 原 webCsrfMiddleware.ts
│
└── bootstrap/
...
> bindPageAction 僅用於 Page handler 與 routing 的配接，不得作為通用 DI helper。

**重命名對照：**

| 原檔案 | 新位置 | 說明 |
|--------|--------|------|
| `inertiaFactory.ts` | `Http/Inertia/createInertiaRequestHandler.ts` | 動詞開頭，明確是工廠函式 |
| `InertiaService.ts` | `Http/Inertia/InertiaRequestHandler.ts` | 說明它做什麼，而非它是什麼 |
| `SharedDataMiddleware.ts` | `Http/Inertia/SharedPropsBuilder.ts` | 職責是建立共用 props |
| `pathUtils.ts` | `Http/Routing/routePath.ts` | 明確描述 URL path 工具 |
| `webCsrfMiddleware.ts` | `Http/Security/CsrfMiddleware.ts` | 歸入安全子目錄 |
...
| `src/Pages/routing/inertiaFactory.ts` | `src/Website/Http/Inertia/createInertiaRequestHandler.ts` |
| `src/Pages/InertiaService.ts` | `src/Website/Http/Inertia/InertiaRequestHandler.ts` |
| `src/Pages/routing/withInertiaPage.ts` | `src/Website/Http/Inertia/withInertiaPage.ts` |
| `src/Pages/SharedDataMiddleware.ts` | `src/Website/Http/Inertia/SharedPropsBuilder.ts` |
...
| `src/Pages/routing/webCsrfMiddleware.ts` | `src/Website/Http/Security/CsrfMiddleware.ts` |
