...
| **Pages / Inertia** — `src/Pages/**` page classes, routing glue, `InertiaService` | Same as Presentation: **class-level** purpose (which path / React page); **each public** `handle` / `store` / `postAction` / `update` with `@param ctx` and `@returns`; route registration files (`register*PageRoutes.ts`, `page-routes.ts`) need a **file-level** block describing the declarative routing + DI pattern. |
