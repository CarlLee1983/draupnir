...
| [`background-job-processing.md`](./background-job-processing.md) | **非同步背景任務** - Redis Stream、Worker 模式、延遲任務與重試機制 | 後端、DevOps |
...
#### 🎬 **非同步處理與背景任務**
👉 開始於 [`background-job-processing.md`](./background-job-processing.md)
- Redis Stream 隊列架構
- 背景 Worker 訂閱模式
- 任務重試與失效處理（DLQ）
- 適用場景：Credit 扣費、郵件投遞、報表生成
...
系統引入了 Redis Stream 作為背景任務中心，將耗時操作從主請求鏈中剝離：

```typescript
// ❌ 同步處理耗時任務
await reportService.generate(user)

// ✅ 非同步入隊
await queue.push(new GenerateReportJob(user.id))
```

**適用場景**：
- **Credit**：高併發下的非同步扣費（削峰填谷）
- **Signal**：郵件與通知投遞
- **Reports**：大型 PDF 生成
- **Alerts**：Webhook 觸發
...
週期性任務（如 Credit 過期檢查、報表排程）統一由 `config/schedule.ts` 定義，不再散落在模組內部，提升了可維護性。

### ✅ 主題感知的呈現層

Inertia 層支援動態主題切換（Day/Night），並透過 Middleware 確保使用者偏好在不同裝置間同步。
...
**最後更新**：2026-04-26  
**評審狀態**：✅ V1.1 非同步架構升級完成（15 模組）；新增 Redis Stream 背景任務與分離式排程系統
