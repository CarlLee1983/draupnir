...
├── Application/
│   └── Services/
│       └── JwtTokenService.ts          ← 核心封裝（118 行）
└── Domain/
    └── ValueObjects/
        └── AuthToken.ts                 ← 值物件（不依賴 jsonwebtoken）
```

> ✅ **封裝度極佳**：`jsonwebtoken` 只在 `JwtTokenService.ts` 一個檔案裡被 import，完全符合 DDD Application 層的封裝原則。
...
| API | 位置 | 功能 |
|-----|------|------|
| `jwt.sign(payload, secret)` | JwtTokenService.ts:40, 58 | 簽發 token（預設 HS256） |
| `jwt.verify(token, secret)` | JwtTokenService.ts:69 | 驗證簽章 + 過期時間 |
| `jwt.decode(token)` | JwtTokenService.ts:81 | 解析但不驗證 |
...
### Public API（`JwtTokenService` 對外介面）

```typescript
class JwtTokenService {
  signAccessToken(payload: TokenSignPayload): AuthToken     // 同步
  signRefreshToken(payload: TokenSignPayload): AuthToken    // 同步
  verify(token: string): TokenPayload | null                // 同步
  decode(token: string): TokenPayload | null                // 同步
  isValid(token: string): boolean                           // 同步
  getTimeToExpire(token: string): number                    // 同步
  isAboutToExpire(token: string): boolean                   // 同步
}
```
...
| 檔案 | 使用的方法 | 所在模組 |
|------|----------|---------|
| `src/Shared/Infrastructure/Middleware/AuthMiddleware.ts` | `verify` | Shared |
...
| `src/Modules/Auth/__tests__/AuthFlow.e2e.test.ts` | `verify` + dynamic `jwt.sign` | 測試 |
| `src/Modules/CliApi/__tests__/ExchangeDeviceCodeService.test.ts` | `new JwtTokenService()` | 測試 |

> **好消息**：所有 production consumer（AuthMiddleware 除外）本來就是 async function，因為它們都涉及 DB 操作。這代表把 `signAccessToken`/`verify` 改成 async 時，**不會往上污染**到 Controller 層。
...
| # | 檔案 | 改動類型 | 估計行數 |
|---|------|---------|---------|
| 1 | `JwtTokenService.ts` | 全面改寫 | ~40 行 |
| 2 | `AuthToken.ts` | **無需修改** | 0 |
| 3 | `AuthMiddleware.ts` | 加 `await` | ~2 行 |
| 4 | `LoginUserService.ts` | 加 `await` x2 | ~2 行 |
| 5 | `RefreshTokenService.ts` | 加 `await` x2 | ~2 行 |
| 6 | `ExchangeDeviceCodeService.ts` | 加 `await` x2 | ~2 行 |
| 7 | `AuthServiceProvider.ts` | **無需修改** | 0 |
| 8 | `LoginUserService.test.ts` | 加 `await` | ~1 行 |
| 9 | `AuthFlow.e2e.test.ts` | 改 dynamic import + `await` | ~3 行 |
| 10 | `ExchangeDeviceCodeService.test.ts` | 可能需加 `await` | ~1 行 |
...
這些是 `JwtTokenService` public API 的破壞性變更：
...
> ✅ **外部 API 穩定性**：雖然 service 層有 breaking change，但**HTTP API 的 request/response 完全不變**。用戶端感知不到任何差異。
...
1. ✅ 清理 **362 個 Biome warnings**（影響更廣、CI 紅綠更乾淨）
...
3. **測試保險**：現有 46 個測試會先於 production 抓到問題
...
- [ ] 執行 baseline：`bun test src/Modules/Auth tests/ > /tmp/before.txt`
- [ ] 安裝 jose：`bun add jose && bun remove jsonwebtoken @types/jsonwebtoken`

### 實作步驟（按順序）

#### Step 1: 改寫 `JwtTokenService.ts`
...
- [ ] `AuthMiddleware.ts`（原為 sync handle → 確認已是 async）
- [ ] `LoginUserService.ts:84, 91`
- [ ] `RefreshTokenService.ts:52, 91`
- [ ] `ExchangeDeviceCodeService.ts:65, 72`

#### Step 4: 修測試

- [ ] `LoginUserService.test.ts:54`：`await jwtTokenService.verify(...)`
- [ ] `AuthFlow.e2e.test.ts:100, 124`：加 `await`
- [ ] `AuthFlow.e2e.test.ts:230-241`：改用 `SignJWT` 製造過期 token
- [ ] `ExchangeDeviceCodeService.test.ts`：檢查所有 sign/verify 呼叫

#### Step 5: 驗證

- [ ] `bun run typecheck` 通過
- [ ] `bun run lint` 通過（0 errors）
- [ ] `bun run test` 通過（46/46）
- [ ] `bun run build` 成功
...
### 程式碼參考
- `src/Modules/Auth/Application/Services/JwtTokenService.ts` — 現有實作
