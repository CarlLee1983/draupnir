...
**自動檢查**：`scripts/check-banned-imports.sh`（透過 pre-commit hook 執行）
...
每次 `git commit` 前會自動執行 `scripts/check-banned-imports.sh`，檢查：
...
1. **本文件** — 新增項目描述與替代方案
2. **`scripts/check-banned-imports.sh`** — 新增檢查邏輯
3. **`docs/DEPENDENCY_OPTIMIZATION_TODO.md`** — 在對應 Task 中記錄
4. **Git commit** — 使用 `chore: [deps] 更新禁用 imports 清單` 的格式
