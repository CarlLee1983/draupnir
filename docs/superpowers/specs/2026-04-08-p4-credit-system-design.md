...
HandleBalanceDepletedService:
  ...
  3. 對每個 Key：BifrostClient.updateRateLimit(virtualKeyId, { rpm: 0, tpm: 0 })
  4. 標記每個 Key 狀態為 SUSPENDED_NO_CREDIT
```
...
HandleCreditToppedUpService:
  1. 從事件取得 orgId
  2. 查詢該 Org 所有 status = SUSPENDED_NO_CREDIT 的 ApiKeys
  3. 對每個 Key：讀取 Key 原始 rate limit 設定
  4. BifrostClient.updateRateLimit(virtualKeyId, originalRateLimit)
  5. 標記 Key 狀態恢復為 ACTIVE
```
...
| `lastSyncedAt` | timestamp | 上次同步的時間戳 |
| `lastBifrostLogId` | string | 上次處理的最後一筆 log ID |
...
SyncBifrostUsageService.execute():
  ...
  2. 呼叫 BifrostClient.getUsageLogs({ after: lastSyncedAt })
  3. 過濾已存在的 bifrostLogId（冪等）
  4. 開啟 DB 交易：
     a. 批次寫入 UsageRecord
     b. 對每筆 record 透過 UsagePricingCalculator 計算 creditCost
     c. 按 orgId 分組，呼叫 CreditDeductionService.deduct() 扣款
     d. 推進 SyncCursor（更新 lastSyncedAt + lastBifrostLogId）
  5. 交易提交
  6. 呼叫 DetectUsageAnomalyService 檢查異常
```
...
| `GET /dashboard/cost` | 本期費用、上期費用、趨勢 | CreditTransaction (DEDUCTION) 聚合 |
