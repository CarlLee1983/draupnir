...
- [x] **SDK-01**: `packages/bifrost-sdk/` exists as a Bun workspace package with its own `package.json`, `tsconfig.json`, and `README.md`
- [x] **SDK-02**: `BifrostClient`, `BifrostClientConfig`, types, errors, and retry logic are moved from `src/Foundation/Infrastructure/Services/BifrostClient/` into `packages/bifrost-sdk/src/` and re-exported from its `index.ts`
...
- [x] **SDK-06**: `packages/bifrost-sdk/` has at least one self-contained smoke test that builds and runs without importing anything from Draupnir `src/`
...
- [x] **QUAL-04**: HTTP routes and request/response shapes unchanged — smoke-check via `routes-connectivity.e2e.ts` / `routes-existence.e2e.ts`
