...
- [x] **Phase 4: SDK Extraction** - Move `BifrostClient` into `packages/bifrost-sdk/` Bun workspace package; update all imports (completed 2026-04-10)
...
**Success Criteria** (what must be TRUE):
  ...
  5. Full Bun unit + feature test suite passes (including `routes-connectivity.e2e.ts` and `routes-existence.e2e.ts`) — all HTTP routes respond correctly
**UI hint**: no
**Plans**: 5 plans
Plans:
