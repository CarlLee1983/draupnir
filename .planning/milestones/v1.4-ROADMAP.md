...
- ✅ **v1.4 Hardening & Refinement** — Phases 18-20 (shipped 2026-04-13)
...
- [x] **Phase 19: Alerts Module Decoupling (3 plans)** (completed 2026-04-13)
  ...
  **Plans:** 3 plans
  Plans:
  - [x] 19-01-PLAN.md — ORM 解耦：4 個 Alerts repos 改用 IDatabaseAccess + IQueryBuilder；denormalize alert_deliveries 的 org_id/month/tier；DI binding 去除 drizzle 前綴
  - [x] 19-02-PLAN.md — 依賴收斂：IAlertRecipientResolver port + 實作；SendAlertService / EvaluateThresholdsService object-literal ctor；MODULE.md
  - [x] 19-03-PLAN.md — Notifier 統一 + DI-less fakes：IAlertNotifier port + Email/Webhook notifiers；ResendDeliveryService 接 notifierRegistry；`__tests__/fakes/` 共用 InMemory fixtures；刪除 DispatchAlertWebhooksService
- [x] **Phase 20: CI Verification Guardrails (5 plans)**
  ...
  **Plans:** 5 plans
  ...
  - [x] 20-03-PLAN.md — Gap closure: 修復 9 個失敗 unit tests（Cluster A 替換 vi.mocked → bun:test；Cluster B Profile 測試容忍 auto-generated UUID；Cluster C 修復 MemoryQueryBuilder.insertOrIgnore 冪等測試）（completed 2026-04-13）
  - [x] 20-04-PLAN.md — Gap closure: 應用 `biome check --write` 安全自動修復 231 format + 368 organizeImports 違規（~738 檔，獨立 style commit 便於審查）（completed 2026-04-13）
  - [x] 20-05-PLAN.md — Gap closure: interactive rebase reword commit `a583918` 將 body 包裹至 100 字元 + force-push with lease（completed 2026-04-13）
...
| 19. Alerts Decoupling | v1.4 | 3/3 | Complete    | 2026-04-13 |
| 20. CI Guardrails | v1.4 | 5/5 | Complete    | 2026-04-13 |
