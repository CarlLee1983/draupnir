...
async function getAuthForOperation(op: Operation): Promise<string | null> {
  ...
  const role =
    op.testRole === 'admin' || op.path.startsWith('/api/organizations/{') ? 'admin' : 'user'
  ...
  describe(`${op.method.toUpperCase()} ${op.path}`, () => {
    ...
    if (op.requiresAuth) {
      it('未帶 token 回傳 401', async () => {
        ...
        const res = await client.request(
          { method: op.method, path },
          { body: requestBody, auth: null, headers },
        )
    ...
    const skipRequiredBodyTest =
      op.method === 'post' &&
      op.path === '/api/auth/refresh' &&
      op.requiredFields.includes('refreshToken')
