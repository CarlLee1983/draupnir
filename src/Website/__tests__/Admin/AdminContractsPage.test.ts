...
describe('AdminContractsPage', () => {
  ...
  test('authenticated admin request renders with correct component (PAGE-01)', async () => {
    ...
    expect(captured.lastCall?.component).toBe('Admin/Contracts/Index')
    expect(captured.lastCall?.props.contracts).toBeDefined()
    expect(captured.lastCall?.props.meta).toBeDefined()
  })
})
