...
import type {
  CreateVirtualKeyRequest, UpdateVirtualKeyRequest, BifrostVirtualKey,
  VirtualKeyResponse, VirtualKeyListResponse,
  BifrostLogsQuery, BifrostLogsResponse, BifrostLogsStats,
  BifrostModel, BifrostModelsQuery, BifrostModelsResponse,
} from './types'
...
export class BifrostClient {
  ...
  async getVirtualKey(vkId: string): Promise<BifrostVirtualKey> {
    const response = await this.get<VirtualKeyResponse>(`/api/governance/virtual-keys/${encodeURIComponent(vkId)}`)
    return response.virtual_key
  }

  async updateVirtualKey(vkId: string, request: UpdateVirtualKeyRequest): Promise<BifrostVirtualKey> {
    const response = await this.put<VirtualKeyResponse>(`/api/governance/virtual-keys/${encodeURIComponent(vkId)}`, request)
    return response.virtual_key
  }
