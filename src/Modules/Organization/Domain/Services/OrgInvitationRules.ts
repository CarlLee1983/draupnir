 ...
 */
export const OrgInvitationRules = {
  /**
   * 驗證邀請的 email 與使用者 email 一致。
   * @throws Error 'EMAIL_MISMATCH' 如不一致
   */
  assertEmailMatches(invitation: OrganizationInvitation, userEmail: string): void {
   ...
   */
  assertNotAlreadyMember(existingMembership: OrganizationMember | null): void {
