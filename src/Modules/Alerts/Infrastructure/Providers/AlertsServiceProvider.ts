...
export class AlertsServiceProvider extends ModuleServiceProvider implements IRouteRegistrar {
  ...
  protected override registerApplicationServices(container: IContainer): void {
    container.bind(
      ...
      (c: IContainer) =>
        new AlertRecipientResolverImpl({
    ...
    wireBind(container, 'listWebhookEndpointsService', ListWebhookEndpointsService, [
      'webhookEndpointRepository',
    ])
    wireBind(container, 'updateWebhookEndpointService', UpdateWebhookEndpointService, [
      'webhookEndpointRepository',
    ])
    wireBind(container, 'rotateWebhookSecretService', RotateWebhookSecretService, [
      'webhookEndpointRepository',
    ])
    wireBind(container, 'deleteWebhookEndpointService', DeleteWebhookEndpointService, [
      'webhookEndpointRepository',
    ])
    ...
    container.bind(
      ...
      (c: IContainer) =>
        new TestWebhookEndpointService({
          repo: c.make('webhookEndpointRepository') as IWebhookEndpointRepository,
          dispatcher: c.make('webhookDispatcher') as IWebhookDispatcher,
        }),
    )
    container.bind(
      ...
      (c: IContainer) =>
        new GetAlertHistoryService({
          eventRepo: c.make('alertEventRepository') as IAlertEventRepository,
          deliveryRepo: c.make('alertDeliveryRepository') as IAlertDeliveryRepository,
        }),
    )
    container.bind(
      'resendDeliveryService',
      (c: IContainer) =>
        new ResendDeliveryService({
          deliveryRepo: c.make('alertDeliveryRepository') as IAlertDeliveryRepository,
          eventRepo: c.make('alertEventRepository') as IAlertEventRepository,
  ...
  protected override registerControllers(container: IContainer): void {
    wireBind(container, 'alertController', AlertController, [
      'setBudgetService',
      'getBudgetService',
    ])
    container.bind(
  ...
  registerRoutes(context: IRouteContext): void {
    ...
    const webhookController = context.container.make(
      'webhookEndpointController',
    ) as WebhookEndpointController
    const historyController = context.container.make(
      'alertHistoryController',
    ) as AlertHistoryController
