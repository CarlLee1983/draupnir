...
export class GetCostTrendsService {
  ...
  async execute(query: DashboardAnalyticsQuery): Promise<CostTrendsResponse> {
    try {
      ...
      if (query.callerSystemRole === 'admin' || membershipRole === 'manager') {
        const buckets = await this.usageRepository.queryDailyCostByOrg(query.orgId, range)
      ...
      const buckets =
        keyIds.length > 0 ? await this.usageRepository.queryDailyCostByKeys(keyIds, range) : []
