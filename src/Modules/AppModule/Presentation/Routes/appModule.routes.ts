...
export function registerAppModuleRoutes(
  ...
  router.post('/api/modules', [createRoleMiddleware('admin')], RegisterModuleRequest, (ctx) =>
  ...
  router.post(
    '/api/organizations/:orgId/modules/subscribe',
    [createRoleMiddleware('admin')],
    SubscribeModuleRequest,
    (ctx) => controller.subscribe(ctx),
  )
  router.delete(
    '/api/organizations/:orgId/modules/:moduleId',
    [createRoleMiddleware('admin')],
    (ctx) => controller.unsubscribe(ctx),
  )
